...
public class Main{
    public static void main(String[] args){
        ...
        */
        ...
        char[][] charBoard = BoardFormatConverter.boardFormatToCharBoard(board);

        for (int col=0; col<8; col++){
            for(int row=0; row < 8; row++){
                System.out.printf("%c ", charBoard[col][row]);
            }
            System.out.println("");
        }

        System.out.printf("%s\n", BoardFormatConverter.convertCharBoardToFEN(charBoard));
    }
}